...
#ifndef _RQC_SIPHASH_H_INCLUDED_
#define _RQC_SIPHASH_H_INCLUDED_
...
#define RQC_SIPHASH_KEY_SIZE        16
#define RQC_SIPHASH_C_ROUNDS        2
#define RQC_SIPHASH_D_ROUNDS        4
#define RQC_DEFAULT_HASH_SIZE       8

/* save siphash context */
typedef struct rqc_siphash_ctx {
    /* v0 v1 v2 v3  */
    uint64_t v0;
    uint64_t v1;
    uint64_t v2;
...
static inline int
rqc_siphash_init(rqc_siphash_ctx_t *ctx, const unsigned char *k, size_t key_len,
    size_t hash_size, int crounds, int drounds)
{
    uint64_t k0, k1;
    if (key_len != RQC_SIPHASH_KEY_SIZE) {
        return RQC_ERROR;
    }
    ...
    /* default: SipHash-2-4 */
    if (crounds == 0) {
        ctx->crounds = RQC_SIPHASH_C_ROUNDS;
    } else {
        ctx->crounds = crounds;
    }
    if (drounds == 0) {
        ctx->drounds = RQC_SIPHASH_D_ROUNDS;
    } else {
        ctx->drounds = drounds;
    }
    return RQC_OK;
...
static inline int
rqc_siphash(rqc_siphash_ctx_t *ctx, const uint8_t *in, size_t inlen, uint8_t *out, size_t outlen)
{
...
#endif /* _RQC_SIPHASH_H_INCLUDED_ */
