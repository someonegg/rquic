...
#ifndef _RQC_HASH_H_INCLUDED_
#define _RQC_HASH_H_INCLUDED_
...
rqc_hash_string(const u_char *data, size_t len)
{
...
rqc_murmur_hash2(u_char *data, size_t len)
{
    uint32_t  h, k;
...
static const u_char *rqc_md5_body(rqc_md5_t *ctx, const u_char *data, size_t size);

static inline void
rqc_md5_update(rqc_md5_t *ctx, const void *data, size_t size)
{
    size_t  used, free;
    ...
    if (size >= 64) {
        data = rqc_md5_body(ctx, data, size & ~(size_t)0x3f);
        size &= 0x3f;
    }
...
static inline void
rqc_md5_final(u_char result[16], rqc_md5_t *ctx)
{
    size_t  used, free;
...
#if (RQC_LITTLE_ENDIAN && RQC_NONALIGNED)
...
static const u_char *
rqc_md5_body(rqc_md5_t *ctx, const u_char *data, size_t size)
{
    uint32_t       a, b, c, d;
 ...
 */
static inline uint64_t
rqc_hash_optimize(const u_char *data, size_t len)
{
    ...
    if (left_len) {
        h = h ^ rqc_hash_string((const u_char *)end, left_len);
    }

    return h;
}

#endif /* _RQC_HASH_H_INCLUDED_ */
