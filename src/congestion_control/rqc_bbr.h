...
#ifndef _RQC_BBR_H_INCLUDED_
#define _RQC_BBR_H_INCLUDED_
...
#define RQC_BBR_DISABLE_CWND_AI
...
typedef enum {
    ...
    BBR_IN_RECOVERY,
} rqc_bbr_recovery_mode;

typedef struct rqc_bbr_s {
    /* Current mode */
    rqc_bbr_mode           mode;
    /* State of the sender */
    ...
    rqc_bbr_recovery_mode  recovery_mode;
    ...
    rqc_bool_t             lt_bw_enabled;
    uint64_t               lt_last_lost_pkt;
    uint64_t               lt_last_delivered_bytes;
    rqc_usec_t             lt_last_stamp;
    uint64_t               lt_bw;
    rqc_bool_t             lt_is_sampling;
    rqc_bool_t             lt_use_bw;
    uint16_t               lt_rtt_cnt;

    rqc_bool_t             ignore_app_limit;

} rqc_bbr_t;
extern const rqc_cong_ctrl_callback_t rqc_bbr_cb;
