/**
 * @copyright Copyright (c) 2022, Alibaba Group Holding Limited
 */

#include "rqc_hq.h"
#include "rqc_hq_request.h"
#include "rqc_hq_ctx.h"
#include "rqc_hq_conn.h"
#include "rqc_hq_defs.h"

#include "src/common/rqc_common_inc.h"
#include "src/transport/rqc_engine.h"
#include "src/transport/rqc_stream.h"
#include "src/transport/rqc_conn.h"

#define RQC_HQ_REQUEST_BASE_LEN             7
#define RQC_HQ_REQUEST_MAX_LEN              512
#define RQC_HQ_REQUEST_RESOURCE_MAX_LEN     256
#define RQC_HQ_RESPONSE_MAX_LEN             4096

typedef struct rqc_hq_request_s {
    /**
     * transport-level callback functions, taken over by hq request
     */
    rqc_hq_request_callbacks_t *hqr_cbs;

    /**
     * app-level user_data, which will be used when hq-level callback functions are triggered
     */
    void                       *user_data;

    /*
     * quic-level stream, we will always make rqc_hq_request_t instance the user_data of
     * rqc_stream_t. if the user_data of stream callback functions is NULL, it means
     * rqc_hq_request_t has not been created yet.
     */
    rqc_stream_t               *stream;

    /**
     * send buffer
     */
    uint8_t                    *send_buf;
    size_t                      send_buf_len;
    size_t                      sent_cnt;

    /**
     * request recv buffer
     */
    uint8_t                    *req_recv_buf;
    size_t                      recv_buf_len;
    size_t                      recv_cnt;

    /**
     * buffer for read request
     */
    uint8_t                    *resource_buf;
    size_t                      resource_buf_sz;
    size_t                      resource_read_offset;
    uint8_t                     fin;
} rqc_hq_request_s;

/* active create request, used by client */
rqc_hq_request_t *
rqc_hq_request_create(rqc_engine_t *engine, rqc_hq_conn_t *hqc, const rqc_cid_t *cid,
    void *user_data)
{
    rqc_hq_request_t *hqr = NULL;
    rqc_stream_t *stream = NULL;

    /* malloc hqr */
    hqr = rqc_calloc(1, sizeof(rqc_hq_request_t));
    if (NULL == hqr) {
        PRINT_LOG("malloc error|");
        return NULL;
    }

    hqr->hqr_cbs = &hqc->hqr_cbs;

    /* create stream, make hqr the user_data of rqc_stream_t */
    stream = rqc_stream_create(engine, cid, NULL, hqr);
    if (NULL == stream) {
        PRINT_LOG("create transport-level stream error");
        goto fail;
    }

    hqr->stream = stream;
    hqr->user_data = user_data; /* make app-level user_data the user_data rqc_hq_request_t */

    return hqr;

fail:
    if (hqr) {
        rqc_hq_request_destroy(hqr);
    }

    return NULL;
}

/* passive create request, used by server */
rqc_hq_request_t *
rqc_hq_request_create_passive(rqc_stream_t *stream)
{
    rqc_hq_request_t *hqr = rqc_calloc(1, sizeof(rqc_hq_request_t));
    if (NULL == hqr) {
        return NULL;
    }

    rqc_hq_conn_t *hqc = rqc_get_conn_alp_user_data_by_stream(stream);

    hqr->hqr_cbs = &hqc->hqr_cbs;

    /* make hqr the user_data of rqc_stream_t */
    rqc_stream_set_user_data(stream, hqr);

    hqr->stream = stream;
    hqr->user_data = NULL;  /* app-level user_data is temporary NULL */

    return hqr;
}

void
rqc_hq_request_destroy(rqc_hq_request_t *hqr)
{
    if (hqr) {
        if (hqr->send_buf) {
            rqc_free(hqr->send_buf);
            hqr->send_buf = NULL;
        }

        if (hqr->req_recv_buf) {
            rqc_free(hqr->req_recv_buf);
            hqr->req_recv_buf = NULL;
        }

        if (hqr->resource_buf) {
            rqc_free(hqr->resource_buf);
            hqr->resource_buf = NULL;
        }

        rqc_free(hqr);
    }
}

rqc_int_t
rqc_hq_request_close(rqc_hq_request_t *hqr)
{
    rqc_int_t ret = rqc_stream_close(hqr->stream);
    if (ret != RQC_OK) {
        PRINT_LOG("|quic stream close fail|ret:%d|stream_id:%"PRIu64"|", ret, hqr->stream->stream_id);
        return ret;
    }

    return RQC_OK;
}

ssize_t
rqc_hq_request_send_data(rqc_hq_request_t *hqr, const uint8_t *data, size_t len, uint8_t fin)
{
    ssize_t ret = 0;
    while (hqr->sent_cnt < hqr->send_buf_len) {
        ret = rqc_stream_send(hqr->stream, hqr->send_buf + hqr->sent_cnt,
                              hqr->send_buf_len - hqr->sent_cnt, 1);
        if (ret < 0) {
            if (ret == -RQC_EAGAIN) {
                return 0;

            } else {
                PRINT_LOG("|send req error|ret:%zd", ret);
                return ret;
            }
        }

        hqr->sent_cnt += ret;
    }

    return ret;
}

ssize_t
rqc_hq_request_send_req(rqc_hq_request_t *hqr, const char *resource)
{
    ssize_t ret = 0;

    if (hqr->send_buf) {
        PRINT_LOG("|request exists on stream|stream_id:%"PRIu64"|", hqr->stream->stream_id);
        return -RQC_EFATAL;
    }

    size_t max_req_buf_len = strlen(resource) + RQC_HQ_REQUEST_BASE_LEN;
    hqr->send_buf = rqc_malloc(max_req_buf_len);
    if (NULL == hqr->send_buf) {
        PRINT_LOG("|request exists on stream|stream_id:%"PRIu64"|", hqr->stream->stream_id);
        return -RQC_EMALLOC;
    }

    /* format HQ request */
    hqr->send_buf_len = snprintf(hqr->send_buf, max_req_buf_len, "GET %s\r\n", resource);
    ret = rqc_hq_request_send_data(hqr, hqr->send_buf, hqr->send_buf_len, 1);
    if (ret < 0) {
        PRINT_LOG("|send request error|ret: %zd|", ret);
        return ret;
    }

    return strlen(resource);
}

ssize_t
rqc_hq_request_send_rsp(rqc_hq_request_t *hqr, const uint8_t *res_buf, size_t res_buf_len,
    uint8_t fin)
{
    ssize_t ret = rqc_stream_send(hqr->stream, (unsigned char *)res_buf, res_buf_len, fin);
    if (ret == -RQC_EAGAIN) {
        ret = 0;
    }

    return ret;
}

ssize_t
rqc_hq_parse_req(rqc_hq_request_t *hqr, char *res, size_t sz, uint8_t *fin)
{
    char method[16] = {0};
    int ret = sscanf(hqr->req_recv_buf, "%s %s", method, res);
    if (ret <= 0) {
        PRINT_LOG("|parse hq request failed: %s", hqr->req_recv_buf);
        return -RQC_EPROTO;
    }

    int request_line_len = strlen(method) + strlen(res) + 1; /* method + ' ' + path */
    if (request_line_len + 2 <= hqr->recv_buf_len
        && (*(hqr->req_recv_buf + request_line_len) == '\r')
        && (*(hqr->req_recv_buf + request_line_len + 1) == '\n'))
    {
        /* check CR LF for hq request line */
        *fin = 1;
        PRINT_LOG("|hq recv CR LF|");
    }

    return strlen(res);
}

ssize_t
rqc_hq_request_recv_req(rqc_hq_request_t *hqr, char *res_buf, size_t buf_sz, uint8_t *fin)
{
    if (hqr->req_recv_buf == NULL) {
        hqr->req_recv_buf = rqc_malloc(RQC_HQ_REQUEST_MAX_LEN);
        if (NULL == hqr->req_recv_buf) {
            return -RQC_EMALLOC;
        }

        hqr->recv_buf_len = RQC_HQ_REQUEST_MAX_LEN;
    }

    *fin = 0;
    ssize_t read = 0;
    do {
        read = rqc_stream_recv(hqr->stream, hqr->req_recv_buf + hqr->recv_cnt,
                               hqr->recv_buf_len - hqr->recv_cnt, &hqr->fin);
        if (read == -RQC_EAGAIN) {
            break;

        } else if (read < 0) {
            PRINT_LOG("|rqc_stream_recv error %zd|", read);
            return 0;
        }

        hqr->recv_cnt += read;
        if (hqr->recv_cnt == hqr->recv_buf_len && hqr->fin) {
            PRINT_LOG("|impossible resource len!|");
            return -RQC_EFATAL;
        }

    } while (read > 0 && !hqr->fin);

    if (NULL == hqr->resource_buf) {
        hqr->resource_buf = rqc_malloc(RQC_HQ_REQUEST_RESOURCE_MAX_LEN);
        if (NULL == hqr->resource_buf) {
            return -RQC_EMALLOC;
        }

        hqr->resource_buf_sz = RQC_HQ_REQUEST_RESOURCE_MAX_LEN;
    }

    uint8_t req_fin = 0;
    read = rqc_hq_parse_req(hqr, hqr->resource_buf, RQC_HQ_REQUEST_RESOURCE_MAX_LEN, &req_fin);
    if (read <= 0) {
        if (!hqr->fin) {
            /* return until all request bytes are received in the current request */
            return RQC_OK;
        } else {
            return -RQC_EPROTO;
        }
    }

    /* return until all request bytes are received in the current request */
    if (!hqr->fin && !req_fin) {
        return RQC_OK;
    }

    if (hqr->resource_read_offset < strlen(hqr->resource_buf)) {
        read = (ssize_t)snprintf(res_buf, buf_sz, "%s", hqr->resource_buf + hqr->resource_read_offset);
        hqr->resource_read_offset += read;
        *fin = (hqr->fin || req_fin);
    }

    return read;
}

ssize_t
rqc_hq_request_recv_rsp(rqc_hq_request_t *hqr, char *res_buf, size_t buf_sz, uint8_t *fin)
{
    ssize_t ret = rqc_stream_recv(hqr->stream, res_buf, buf_sz, fin);
    if (ret > 0) {
        hqr->recv_cnt += ret;
    }
    return ret;
}

void
rqc_hq_request_set_user_data(rqc_hq_request_t *hqr, void *user_data)
{
    hqr->user_data = user_data;
}

rqc_stream_stats_t
rqc_hq_request_get_stats(rqc_hq_request_t *hqr)
{
    return rqc_stream_get_stats(hqr->stream);
}

int
rqc_hq_stream_create_notify(rqc_stream_t *stream, void *strm_user_data)
{
    rqc_hq_request_t *hqr = (rqc_hq_request_t *)strm_user_data;
    /* the stream of server is passive created, and it's strm_user_data is NULL */
    if (hqr == NULL) {
        hqr = rqc_hq_request_create_passive(stream);
        if (NULL == hqr) {
            return -RQC_EMALLOC;
        }
    }

    /* notify to app-level */
    if (hqr->hqr_cbs->req_create_notify) {
        return hqr->hqr_cbs->req_create_notify(hqr, hqr->user_data);
    }

    return RQC_OK;
}

int
rqc_hq_stream_write_notify(rqc_stream_t *stream, void *user_data)
{
    rqc_hq_request_t *hqr = (rqc_hq_request_t *)user_data;

    ssize_t ret = rqc_hq_request_send_data(hqr, hqr->send_buf + hqr->sent_cnt,
        hqr->send_buf_len - hqr->sent_cnt, 1);
    if (ret < 0) {
        return (int)ret;
    }

    if (hqr->hqr_cbs->req_write_notify) {
        return hqr->hqr_cbs->req_write_notify(hqr, hqr->user_data);
    }

    return RQC_OK;
}

int
rqc_hq_stream_read_notify(rqc_stream_t *stream, void *user_data)
{
    rqc_hq_request_t *hqr = (rqc_hq_request_t *)user_data;
    if (hqr->hqr_cbs->req_read_notify) {
        return hqr->hqr_cbs->req_read_notify(hqr, hqr->user_data);
    }

    return RQC_OK;
}

int
rqc_hq_stream_close_notify(rqc_stream_t *stream, void *user_data)
{
    rqc_hq_request_t *hqr = (rqc_hq_request_t *)user_data;
    if (hqr->hqr_cbs && hqr->hqr_cbs->req_close_notify) {
        hqr->hqr_cbs->req_close_notify(hqr, hqr->user_data);
    }

    rqc_hq_request_destroy(hqr);
    return RQC_OK;
}

/**
 * transport callback
 */
const rqc_stream_callbacks_t hq_stream_callbacks = {
    .stream_create_notify   = rqc_hq_stream_create_notify,
    .stream_write_notify    = rqc_hq_stream_write_notify,
    .stream_read_notify     = rqc_hq_stream_read_notify,
    .stream_close_notify    = rqc_hq_stream_close_notify,
};
